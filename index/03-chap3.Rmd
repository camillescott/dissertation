...
databases, they can be difficult to install and configure, especially for those with limited bioinformatics expertise. To make matters worse, many annotators trade performance for familiarity in their supporting software (eg. BLAST).
...
dammit provides a solution for users wanting local installation, good performance, and ease-of-use.
...
`pydoit` [@url:http://pydoit.org], with each step in the database preparation and annotation process decomposed
into a `pydoit` task, allowing for easy dependency tracking and resume capability.
Version 2 is a complete rewrite using Snakemake [@doi:https://doi.org/10.12688/f1000research.29032.1]; while the underlying
pipeline remains mostly the same, the `pydoit` tasks have been converted into reusable Snakemake wrappers and the command line
interface wraps the Snakemake CLI. Snakemake is widely used within the field and has a rapidly growing community.
It also enables first-class support of High Performance Compute (HPC) system schedulers, 
numpy and pandas \cite{van_der_walt_numpy_2011}\cite{mckinney_data_2010} are leveraged
