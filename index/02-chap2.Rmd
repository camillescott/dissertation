...
Distance curves of the same samples as \@ref(fig:fig-chap2-sourmash-diginorm) at digitally-normalize maximum coverage of $20$.
...
\@ref(fig:fig-chap2-sourmash-diginorm) and \@ref(fig:fig-chap2-sourmash-diginorm-single) show distance curves for regular MinHash sketches with size $N=25000$.
...
The $70m$ sequence whole-genome sample of *C. elegans*, in the bottom left, change in distance stabilizes much sooner.
...
```{r fig-chap2-sourmash-draff-comp, results="asis", echo=FALSE, fig.cap="(ref:fig-chap2-sourmash-draff-comp)", fig.align="center", out.width="100%"}
include_graphics("figure/chap2/sourmash-draff-ref-comparison.png")
```

(ref:fig-chap2-sourmash-draff-comp) **Comparison of MinHash and draff streaming distances as compared to representative sketches.**
draff sketches with $W=31$, $K=9$ and MinHash sketches with $K=31$, $N=25000$ are compared.
The distances are between the sketch at a given time point and the final sketch in the stream, as in the middle figure of \@ref(fig:fig-chap2-sketch-saturation).
...
We posit that MinHash sketches of sufficient size should act similarly to the draff sketch; however, MinHash sketches are bound by worst-case $\mathcal{O}(log(n))$ insertion time, whereas insertion into the draff sketch is always $\mathcal{O}(1)$ (for the implemented exact and AMQ backends), which imposes scaling challenges on MinHash.
...
Finally, there is the question of integrating this streaming method with the streaming compaction described in \@ref(streaming-cdbg).
