...
For some string $s$, let $s[i]$ be the symbol at position $i$, $0$-indexed, $s[i:j]$ be the substring of $s$ starting at position $i$ and not including position $j$, and $\texttt{len}(s)$ be the length of $s$. 
...
Let $\texttt{pre}(s)=s[0:k-1]$ and $\texttt{suf}(s)=s[L-k+1:L]$, that is, they yield the suffix and prefix of $s$.
