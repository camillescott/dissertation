...
The streaming compactor is implemented in the `goetia` \cite{goetia} package, which is hosted on GitHub and available under the MIT license.
...
This functionality is implemented as a C++ library, which can be installed and linked against as a shared library via `bioconda`[@gruning2018bioconda]. High-performance Python bindings are provided using `cppyy`[@cppyy2016], which are used in a corresponding Python library to provide a command line interface (CLI), analysis tools, and unit testing.
